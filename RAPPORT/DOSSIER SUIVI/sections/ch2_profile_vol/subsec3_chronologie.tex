\subsection{Chronologies de vol}

\subsubsection{Note de lecture des chronogrammes}

\noindent La convention de lecture de ces chronogrammes repose sur trois niveaux d'information :

\begin{itemize}
    \item Le premier est la fenêtre temporelle, identifiée par le préfixe "\texttt{FT\_}" et représentée par une zone hachurée :
    elle désigne l'intervalle à l'intérieur duquel un événement donné (détection de séparation, d'allumage, d'apogée, ...) doit
    avoir lieu. Pendant toute cette plage, la logique du lanceur exploite ses mesures et attend que l'ensemble des conditions
    propres à cet événement se réunissent pour que celui-ci puisse survenir. Ce sont donc ces conditions, et non une date
    programmée, qui déterminent si l'événement a lieu et à quel moment. Une fenêtre qui s'écoule sans que la convergence soit
    atteinte se traduit par un événement qui ne survient pas. L'instant réel de déclenchement ne peut par conséquent pas être
    connu à l'avance.\\

    \item Le deuxième niveau, l'événement lui-même, matérialisé par un losange, est donc placé à un instant temporel purement
    théorique choisi dans la fenêtre dans le seul but de rendre la séquence lisible.\\

    \item Le troisième niveau regroupe les actions qui découlent de ces événements (commande d'actionneur, phase de combustion,
...
Ces chronogrammes se lisent ainsi comme un enchaînement logique de conditions, d'événements et de conséquences, dans lequel seuls
l'ordonnancement des étapes et l'amplitude des fenêtres font foi. La convention étant appliquée de façon homogène aux trois
scénarios (\texttt{SCE\_0}, \texttt{SCE\_1}, \texttt{SCE\_2}), leur comparaison directe reste pertinente.
...
\begin{figure}[H]
    ...
    \caption{Exemple de chronologie de vol (scénario 0)}
    \label{fig:timeline_sce0_ex}
\end{figure}
...
\noindent Les chronogrammes présentés ci-dessus reposent sur des horodatages théoriques, mais la logique embarquée est en
réalité paramétrée par un ensemble de temps caractéristiques, dont la dénomination suit une convention systématique. Le
tableau~\ref{tab:timeline_legende} en donne la liste complète.

\begin{itemize}
    \item $T_0$ désigne l'instant de référence, le décollage, auquel tous les autres temps sont rapportés.\\

    \item L'indice grec précise l'étage concerné : $\alpha$ pour le premier étage, $\beta$ pour le second, et $\alpha\beta$
    pour les temps communs aux deux.\\

    \item Un couple noté $\left\{T_x, \Delta T_x\right\}$ désigne une fenêtre de détection, dont l'interval de temps est $[T_x, T_x + \Delta T_x]$.\\

    \item L'indice numérique donne le rang chronologique du temps à l'intérieur de son groupe.\\
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        Nom & Description \\
        \midrule
        \multicolumn{2}{c}{\textit{Etage 1 \& 2}} \\
        \midrule
        $T_0$ & Décollage \\
        $T_{\alpha\beta0}$ & Déverrouillage du système de séparation \\
        $T_{\alpha\beta1}$ & Ouverture des aérofreins \\
        $\left\{T_{\alpha\beta2}, \Delta T_{\alpha\beta2}\right\}$ & Fenêtre de détection de la séparation \\
        $T_{\alpha\beta3}$ & Déverrouillage des ressorts du système de séparation \\
        $\left\{T_{\alpha\beta4}, \Delta T_{\alpha\beta4}\right\}$ & Fenêtre de détection de l'apogée sans séparation (\texttt{SCE\_2}) \\
        \midrule
        \multicolumn{2}{c}{\textit{Etage 1}} \\
        \midrule
        $\left\{T_{\alpha0}, \Delta T_{\alpha0}\right\}$ & Fenêtre de détection de l'apogée avec séparation (\texttt{SCE\_0} ou \texttt{SCE\_1}) \\
        \midrule
        \multicolumn{2}{c}{\textit{Etage 2}} \\
        \midrule
        $\left\{T_{\beta0}, \Delta T_{\beta0}\right\}$ & Fenêtre de détection de l'attitude \\
        $\left\{T_{\beta1}, \Delta T_{\beta1}\right\}$ & Fenêtre de détection de l'allumage du moteur du 2e étage \\
        $\left\{T_{\beta2}, \Delta T_{\beta2}\right\}$ & Fenêtre de détection de l'apogée avec séparation et allumage (\texttt{SCE\_0}) \\
        $\left\{T_{\beta3}, \Delta T_{\beta3}\right\}$ & Fenêtre de détection de l'apogée avec séparation sans allumage (\texttt{SCE\_1}) \\
        \bottomrule
    \end{tabular}
    \caption{Légende des chronogrammes de vol}
    \label{tab:timeline_legende}
\end{table}

La structure du tableau en trois blocs découle directement de la séparation. Tant que les deux étages sont solidaires, ils
partagent la même référence temporelle et les mêmes temps caractéristiques : c'est le groupe $\alpha\beta$, qui couvre
l'ensemble de la séquence de séparation ainsi que le cas où celle-ci n'a pas lieu ($T_{\alpha\beta4}$). Une fois le largage
effectué, chaque étage poursuit avec ses propres temps, ce qui explique que la détection d'apogée se décline ensuite en une
version propre au premier étage ($T_{\alpha0}$) et en deux versions propres au second, selon que son moteur s'est allumé ou
non ($T_{\beta2}$ et $T_{\beta3}$).
...
\begin{itemize}
    \item \textbf{$t = T_0 = 0$~s — Décollage.} La combustion occupe la ligne \texttt{Propulsion} jusqu'à environ 3,5~s ; la
    courte extension hachurée jusqu'à 3,5~s traduit la dispersion attendue sur l'instant d'extinction.\\

    \item \textbf{$t = T_{\alpha\beta0} = 2{,}0$~s — \texttt{sepa\_unlock}.} Commande de déverrouillage du mécanisme de
    séparation. Elle est émise en pleine phase propulsée et non après l'extinction afin d'absorber le délai qui sépare la
    commande de son effet : la ligne \texttt{Actionneurs} montre que le déverrouillage n'est effectivement acquis que vers 3,6~s
    soit après l'extinction du moteur du premier étage.\\

    \item \textbf{$t = 3{,}5$~s — Fin de combustion du premier étage.}\\

    \item \textbf{$t = T_{\alpha\beta1} = 3{,}6$~s — \texttt{airbreakes}.} Déploiement des aérofreins, dont l'action s'achève
    vers 4,1~s. Au début du déploiement, les étages se comportent déjà comme deux corps distincts.\\  

    \item \textbf{$t = T_{\alpha\beta2} = 3{,}6$~s à $7,0~s$ — \texttt{FT\_sepa}.} Fenêtre de détection de la séparation, ouverte
    simultanément sur les deux étages.\\

    \item \textbf{$t = T_{\alpha\beta3} = 4{,}3$~s — \texttt{sepa\_release}.} Ordre de séparation par ressort.\\
    ...
    \item \textbf{$t = T_{\alpha\beta4} = 15{,}9$~s à $19,0$~s — \texttt{FT\_apogee\_detection}.} Fenêtre à l'intérieur de
    laquelle l'apogée doit être constaté.\\
...
\begin{itemize}
    ...
    \item \textbf{$t = T_{\beta0} = 3{,}9$~s à $10,3$~s — \texttt{FT\_attitude}.} Fenêtre pendant laquelle la logique du second
    étage surveille son orientation et sa stabilité, conditions préalables à l'autorisation d'allumage du propulseur.\\
    ...
    \item \textbf{$t = T_{\beta3} = 13{,}8$~s à $16,7$~s — \texttt{FT\_apogee\_detection} (étage 2).} Le second étage dispose désormais de
...
\begin{itemize}
  ...
  \item \textbf{$t = T_{\beta1} = \approx 5{,}0$~s à $10,3$~s — \texttt{FT\_ignition\_detect}.} Fenêtre à l'intérieur de laquelle
  la logique du second étage doit constater, à partir de ses mesures, que l'allumage a effectivement eu lieu.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lllcc}
        \toprule
        Scénario & Étage & Nom & \texttt{FT\_apogee\_detection} & \texttt{apogee\_detected} \\
        \midrule
        SCE\_0  & Étage 1   & $T_{\alpha0}$         & 14,3 -- 17,2~s & 15,7~s \\
        SCE\_0  & Étage 2   & $T_{\beta2}$          & 14,7 -- 17,7~s & 16,2~s \\
        SCE\_1  & Étage 1   & $T_{\alpha0}$         & 14,3 -- 17,2~s & 15,7~s \\
        SCE\_1  & Étage 2   & $T_{\beta3}$          & 13,8 -- 16,8~s & 15,3~s \\
        SCE\_2  & Ensemble  & $T_{\alpha\beta4}$    & 15,9 -- 18,9~s & 17,4~s \\
        \bottomrule
    \end{tabular}
...
\noindent On remarque un ordonnancement des apogées, qui s'explique selon l'énergie et la masse de chaque corps.\\
\begin{enumerate}
