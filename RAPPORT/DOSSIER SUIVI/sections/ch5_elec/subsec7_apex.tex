...
\begin{itemize}
    ...
    mesures, associé à d'autre variables interne, constitue un enregistrement de \SI{127}{\byte} produit à une cadence de
    \SI{100}{\hertz}, soit un flux de \SI{12.7}{\kilo\byte\per\second} (\SI{101.6}{\kilo bit\per\second}).\\
    ...
    \begin{itemize}
        ...
        la durée du vol, soit moins de \SI{4}{\mega\byte} pour un vol de cinq minutes, doit être prévu ;
...
\subsubsection{Le module \gls{APEX} et ses limitations}
...
\subsubsection{Le module ApexEx}
...
\subsubsection{Le système expérimental Apex}
...
\begin{minipage}{0.25\textwidth}
    \begin{figure}[H]
        \begin{subfigure}{1.0\textwidth}
            ...
            \caption{Vue de face du module Pitot.}
            \label{fig:pitot_front}
        \end{subfigure}
        \begin{subfigure}{1.0\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/elec/PCB_Pitot_BACK.jpg}
            \caption{Vue de dos du module Pitot.}
            \label{fig:pitot_back}
        \end{subfigure}
    \end{figure}
\end{minipage}
\hfill
...
Le \acrshort{LORA} est une modulation propriétaire de Semtech reposant sur un étalement de spectre à variation linéaire de
...
Le \acrshort{FSK} (\textit{Frequency-Shift Keying}) code l'information par le décalage de la porteuse autour de sa valeur
nominale. Sans étalement de spectre, il ne bénéficie d'aucun gain de traitement, mais accepte des débits bien supérieurs :
jusqu'à \SI{300}{\kilo bit\per\second} sur le \verb|SX1276|, largement de quoi couvrir les \SI{101.6}{\kilo bit\per\second}
produits par l'expérience. La sensibilité, d'environ \SI{-118}{\dBm} à bas débit, se dégrade toutefois à mesure que celui-ci
augmente.\\
...
filtrage supprime les discontinuités de phase responsables de l'étalement spectral et réduit fortement les remontées de puissance
hors canal, à débit et complexité de démodulation inchangés.
...
\begin{minipage}{0.45\textwidth}
    ...
    \acrshort{GFSK} et \acrshort{OOK}. Deux modules du commerce fondés sur cette puce ont été retenus : le \verb|RFM95W| de HopeRF,
    qui se limite au front-end radio du composant, et le \verb|LoRa1276F30| de NiceRF, qui y ajoute un amplificateur de puissance en
    émission et un amplificateur faible bruit en réception.
\end{minipage}
\hfill
\begin{minipage}{0.5\textwidth}
    \begin{table}[H]
