\subsection{Système électronique général}
...
faciliter le développement, la maintenance et les opérations au sol. Les seules différences notables entre les deux étages sont
au niveau de la ligne de mise à feu du deuxième étage.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/elec/Schema_simple.drawio.pdf}
...
système électronique et doit être vu comme la synthèse référente principale.
...
\begin{itemize}
    ...
    synchronisation. C'est lui qui déclenche les différentes étapes de la mission, comme le déploiement des aérofreins, la
    ...
    puissance nécessaire pour allumer le moteur lorsque le séquenceur l'ordonne mais également d'isolé l'inflamateur moteur de toute
    source d'énergie le reste du temps, afin de maintenir un niveau de sécurié suffisant pour les opérateur pyrotechnique au sol.
    \item \textbf{\texttt{APEX - APEXeX}} : Comme énoncé plus tôt dans ce document, le projet SP-02 embarquera une carte APEX pour
    réaliser l'ensemble des mesures d'enregistrement et d'envoi par télémesure des données de mission. Elle sera accompagnée
...
\begin{itemize}
    ...
    l'arrachage d'un connecteur branché à la rampe de décollage.
    \item \textbf{\texttt{SEQUENCEUR} $\longrightarrow$ \texttt{AEROFREIN}} : Le séquenceur du premier étage contrôle le
    ...
    \item \textbf{\texttt{SEQUENCEUR} $\longrightarrow$ \texttt{SEPARATION}} : Le séquenceur du premier étage contrôle le
    ...
    via deux modules de télémesure, lui permettant d'envoyer des données en temps réel vers le sol et de recevoir des commandes
    depuis le sol.
\end{itemize}

\subsubsection{Cartes électroniques}
