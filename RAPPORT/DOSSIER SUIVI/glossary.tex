...
\newglossaryentry{GNSS_g}{
    ...
    description={
        Système mondial de navigation par satellite : ensemble de constellations de satellites fournissant des données
        \acrshort{GPS} pour la localisation et la synchronisation temporelle à l'échelle mondiale. Les principaux systèmes GNSS
        incluent le GPS (États-Unis), Galileo (Union Européenne), GLONASS (Russie) et BeiDou (Chine).
    },
}
\newglossaryentry{GPS_g}{
    name={Global Positioning System},
    description={
        Système de positionnement global développé par les États-Unis, utilisant une constellation de satellites pour fournir
        des informations de localisation et de temps précises à des récepteurs GPS partout sur Terre. Le terme GPS est souvent
        utilisé de manière générique pour désigner le type de données de positionnement fournies par les systèmes \acrshort{GNSS}.
    },
}
...
\newglossaryentry{ISM_g}{
    name={Industrial, Scientific and Medical},
    description={
        Bande de fréquences radio réservée à un usage industriel, scientifique et médical, utilisée pour les communications
        sans fil, notamment dans les applications de l'\acrshort{IoT}. Les bandes ISM sont réglementées par les autorités de
        télécommunication de chaque pays et sont souvent utilisées pour des technologies telles que le Wi-Fi, le Bluetooth et le
        \acrshort{LORA}.
    },
}
\newglossaryentry{RTOS_g}{
    name={Real-Time Operating System},
    description={
        Système d'exploitation conçu pour traiter les tâches dans des délais stricts, souvent utilisé dans les applications
        critiques où le temps de réponse est essentiel.
    },
}
...
\newacronym{GFSK}{GFSK}{Gaussian Frequency-Shift Keying (Modulation Gaussienne par Déplacement de Fréquence)}
\newacronym{OOK}{OOK}{On-Off Keying (Modulation Par Allumage/Eteignement)}
\newacronym{ET}{ET}{\gls{ET_g}}
\newacronym{GPS}{GPS}{\gls{GPS_g}}
\newacronym{TDMP}{TDMP}{Time Synchronized Mesh Protocol (Protocole de Maillage Synchronisé dans le Temps)}
\newacronym{IMU}{IMU}{Inertial Measurement Unit (Unité de Mesure Inertielle)}
\newacronym{UART}{UART}{Universal Asynchronous Receiver-Transmitter}
\newacronym{CAM}{CAM}{Computer-Aided Manufacturing (Fabrication Assistée par Ordinateur)}
\newacronym{MOSFET}{MOSFET}{Metal-Oxide-Semiconductor Field-Effect Transistor (transistor à effet de champ à structure métal-oxyde-semiconducteur)}
\newacronym{IHF}{IHF}{Interface Homme Fusée}
\newacronym{PWM}{PWM}{Pulse Width Modulation (Modulation de Largeur d'Impulsion)}
...
\newacronym{GPIO}{GPIO}{General Purpose Input/Output (Entrée/Sortie à Usage Général)}
\newacronym{RAM}{RAM}{Random Access Memory (Mémoire à Accès Aléatoire)}
\newacronym{USB}{USB}{Universal Serial Bus (Bus de Communication Universelle)}
\newacronym{SWD}{SWD}{Serial Wire Debug (Débogage par Fil Sérial)}
\newacronym{TDM}{TDM}{Time Division Multiplexing (Multiplexage par Division du Temps)}
\newacronym{ISM}{ISM}{\gls{ISM_g}}
\newacronym{RTOS}{RTOS}{\gls{RTOS_g}}