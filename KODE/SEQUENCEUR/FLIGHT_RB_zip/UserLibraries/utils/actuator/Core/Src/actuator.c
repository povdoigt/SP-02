 ...
 */
static HAL_StatusTypeDef _set_angle_limits(STS_Servo_t *servo, const Actuator_Config_t *config) {
    ...
    // uint16_t cw_units = STS_GetPositionInUnits(min_deg);
    uint16_t cw_units = 0;
    uint8_t  cw[2]    = { (uint8_t)(cw_units & 0xFF), (uint8_t)(cw_units >> 8) };
    res = STS_Servo_WriteRegister(servo, STS_REG_CW_ANGLE_LIMIT_L, cw, 2);
    if (res != HAL_OK) { return res; }

    // uint16_t ccw_units = STS_GetPositionInUnits(max_deg);
    uint16_t ccw_units = 0;
...
Actuator_HomingStatus_t Actuator_HomingProcess(Actuator_t *act) {
    ...
    switch (act->_homing_state) {
        ...
        case _HOMING_STATE_START: {
            ...
            res = STS_Servo_SetGoalSpeed(act->servo, STS_GetSpeedInUnits(act->config.homing_speed_rpm));
            // if (res != HAL_OK) { goto error; }
        ...
        case _HOMING_STATE_CALIBRATE: {
            ...
            res = STS_Servo_PositionCalibration(
                act->servo,
                STS_GetPositionInUnits(
                    act->config.positions_deg[act->config.homing_calibration_idx]
                )
            );
            // if (res != HAL_OK) { goto error; }
...
HAL_StatusTypeDef Actuator_GoToPosition(Actuator_t *act, uint8_t pos_id) {
    ...
    act->current_position = pos_id;

    uint16_t pos_units = STS_GetPositionInUnits(act->config.positions_deg[pos_id]);
...
HAL_StatusTypeDef Actuator_GetCurrentStatus(Actuator_t *act, STS_Servo_Current_raw_t *status) {
